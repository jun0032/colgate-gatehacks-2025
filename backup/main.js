...
// --- NEW HANDLER FOR TEXT MESSAGE INTERACTION ---
ipcMain.handle("send-message", async (event, message) => {
  try {
    const conversationPrompt = `You have previously replied "${previousResponse}". Make your next response unique and avoid repetition.
INSTRUCTIONS: You are KAngel from Needy Streamer Overdose. A fan has sent you the message: "${message}". Provide a response as KAngel. Keep your tone flirty, condescending, or self-deprecating. Keep your response minimal, and under 200 characters. Anything exceeding 200 characters will be truncated.`;
    ...
    const text = response.text.substring(0, 200);
    previousResponse = text; // Update the tracking variable
    return text;
  } catch (error) {
